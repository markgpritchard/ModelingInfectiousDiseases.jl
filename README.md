...
## Use
### Chapter 2
Each programme takes the same form, with the last two digits indicating the programme number. For example, for Programme 2.2, the function `sir_22!` holds the ordinary differential equations, `run_sir_21` runs the model, `print_sir_21` displays the output in the REPL, and `plot_sir_21` plots the output. (Note that for simplicity, all functions use `sir` even if they are not strictly SIR [susceptible--infectious--recovered models].)
