...
export sir81!, run_sir81, dataframe_sir81, plot_sir78, plot_sir78!
...
function plot_sir81!(gl::GridLayout, result::DataFrame; 
        label = "p8.1.jl: SIR model with childhood vaccination")

    ax = Axis(gl[1, 1])
    plot_sir81!(ax, result)
    leg = Legend(gl[1, 2], ax)
end 

function plot_sir81!(ax::Axis, result::DataFrame)
    lines!(ax, result.t / 365, result.S; label = "Susceptible")
    lines!(ax, result.t / 365, result.I; label = "Infectious")
    lines!(ax, result.t / 365, result.R; label = "Resistant")
