...
using CairoMakie, ..MID_2_3, ..MID_2_4
import ..MID_2_3: plot_sir_23_vals, plot_sir_23!
import ..MID_2_4: plot_sir_24_vals, plot_sir_24!

export plot_sirs_23_24
...
function plot_sirs_23_24(; beta = 520 / 365, gamma = 1 / 7, mu = 1 / (70 * 365), nu = 1 / (70 * 365), 
        rho = .5, X0 = .2, Y0 = 1e-6, N0 = 1, duration = 100 * 365)

    sol23 = run_sir_23(; beta, gamma, mu, nu, rho, X0, Y0, N0, duration, saveat = 1)
    sol24 = run_sir_24(; beta, gamma, mu, nu, rho, X0, Y0, N0, duration, saveat = 1)

    fig = Figure()
    ax1 = Axis(fig[1, 1]); ax2 = Axis(fig[2, 1]); ax3 = Axis(fig[3, 1])
    ax4 = Axis(fig[1, 2]); ax5 = Axis(fig[2, 2]); ax6 = Axis(fig[3, 2])

    xs23, X23, Y23, Z23, N23 = plot_sir_23_vals(sol23)
    xs24, X24, Y24, Z24, N24 = plot_sir_24_vals(sol24)

    plot_sir_23!(ax1, ax2, ax3, xs23, X23, Y23, Z23, N23)
    plot_sir_24!(ax4, ax5, ax6, xs24, X24, Y24, Z24, N24)
